...
function Convert-LuxterraStoryDesignDocumentContent {
    ...
    if ($meta.Contains('타입')) { $yamlLines.Add("type: $(ConvertTo-YamlQuotedString $meta['타입'])") }
...
function Convert-LuxterraStoryDesignDocumentsInDirectory {
    ...
    Get-ChildItem -LiteralPath $Directory -Filter *.md -File | ForEach-Object {
        ...
        $converted = Convert-LuxterraStoryDesignDocumentContent -Content $original
...
if ($MyInvocation.InvocationName -ne '.') {
    ...
    Convert-LuxterraStoryDesignDocumentsInDirectory -Directory $args[0]
}
